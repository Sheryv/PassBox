...
public class ExLoginsAdapter extends BaseExpandableListAdapter
{
 ...
 /*            convertView.setTag(view);
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent)
    {
          ...
          ViewHolder viewHolder = null;
        GroupItems group = groupItems.get(groupPosition);
        tempPos = groupPosition;
       // Log.d("group", "this " + group.name + " | " + group.items.size());

        if (convertView == null) {
            par = convertView = inflater.inflate(R.layout.list_group, null);
            ...
            viewHolder.selected = (CheckBox) convertView.findViewById(R.id.g_selectedCb);
            ...
            viewHolder.selected.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                ...
                {
                    int pos = PActivity.getGroupPosByChildView(compoundButton);
                    GroupItems gi = groupItems.get(pos);
                    gi.isSelected = b;
                    setGroup(gi, pos);
                }
            });
        ...
        PActivity.setButtonsVisibility(group, groupPosition, isExpanded, list, convertView);
...
/*        viewHolder.edit.setVisibility(View.INVISIBLE);
    ...
    {
        if(pos >= groupItems.size() )
                Sh.msgl("Error! Index out of range in setting groups");
        else
            if(posChild >= groupItems.get(pos).items.size())
            {
                Sh.msgl("Error! Index out of range in setting items");
                return;
            }
    ...
    public void setList(ArrayList<GroupItems> items)
    {
        groupItems = items;
        notifyDataSetChanged();
    }
    public ArrayList<GroupItems> getList()
    {
        return groupItems;
    }
    @Override
